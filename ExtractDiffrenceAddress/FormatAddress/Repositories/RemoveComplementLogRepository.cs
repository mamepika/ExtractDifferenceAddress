...
namespace ExtractDifferenceAddress.FormatAddress.Repositories
{
    ...
    /// </summary>
    public class RemoveComplementLogRepository : IDisposable
    {
        private SQLiteConnection sqlConnection;

        private string _tableName = "output1";

        public RemoveComplementLogRepository(string filePath)
        {
        ...
        public List<FormatedAddressRecord> FindByFormatLog()
        {
            var query = "SELECT * FROM " + _tableName +
                               " WHERE [FormatLog] LIKE '%RM002%' OR [FormatLog] LIKE '%RM003%'";
            using (var dbCommand = new SQLiteCommand())
            {
                dbCommand.CommandText = query;
                dbCommand.Connection = sqlConnection;
